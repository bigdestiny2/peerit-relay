...
export function createMemoryCore ({
  ...
  return {
    sync: {
      ...
      // Phase D durable directory: the SIGNED head!<appId> record of every outbox
      // in one response, so a fresh visitor bootstraps its rollback floor + full
      // author discovery immediately instead of accumulating them over a session.
      // Untrusted like everything else — the client re-verifies each head's Ed25519
      // signature and takes the highest version across the relay pool. Bounded by
      // maxGroups; capped per call. Durable because heads live in the disk snapshot.
