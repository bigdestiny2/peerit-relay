...
export function createRelayHandler ({ core, auth, allowOrigin = '*', rateLimit = { windowMs: 60000, max: 1200 }, trustProxy = false, sseMaxPerIp = 8, sseMaxTotal = 2000 } = {}) {
  if (!core || !auth) throw new Error('createRelayHandler requires { core, auth }')
  const buckets = new Map() // ip -> { start, count }
  const sseState = { total: 0, perIp: new Map() } // open SSE streams (long-lived; bypass the request limiter)
  ...
  function sse (req, res, url, ip) {
    if (sseState.total >= sseMaxTotal || (sseState.perIp.get(ip) || 0) >= sseMaxPerIp) return sendJson(req, res, 429, { error: 'too many open streams' })
    const channelId = url.searchParams.get('channelId')
    cors(res, req)
    res.writeHead(200, { 'content-type': 'text/event-stream', 'cache-control': 'no-cache', connection: 'keep-alive' })
    res.write(': ok\n\n')
    sseState.total++; sseState.perIp.set(ip, (sseState.perIp.get(ip) || 0) + 1)
    ...
    const cleanup = () => {
      if (closed) return
      closed = true
      clearInterval(ping); try { unsub() } catch {}
      sseState.total = Math.max(0, sseState.total - 1)
      const n = (sseState.perIp.get(ip) || 1) - 1
      if (n <= 0) sseState.perIp.delete(ip); else sseState.perIp.set(ip, n)
    }
  ...
  return async function handler (req, res) {
    ...
    try {
      ...
      if (p === '/api/sync/heads' && req.method === 'POST') { const b = await readBody(req); return sendJson(req, res, 200, core.sync.heads ? await core.sync.heads(b.appIds) : { heads: {} }) }
      if (p === '/api/directory') return sendJson(req, res, 200, core.sync.directory ? await core.sync.directory({ limit: Number(url.searchParams.get('limit')) || 5000 }) : { heads: {} })
      ...
      if (p === '/api/swarm/events' && req.method === 'GET') return sse(req, res, url, ip)
