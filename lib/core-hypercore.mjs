...
export async function createHypercoreCore ({ storage = './relay-store' } = {}) {
  ...
  const sync = {
    ...
    // core.length is monotonic and bumps on every put (incl. edits/overwrites) — a
    // valid per-outbox change-marker for /api/sync/heads, matching the memory core.
    async heads (appIds) { const out = {}; if (Array.isArray(appIds)) for (const a of appIds) { if (typeof a !== 'string') continue; const e = bees.get(a); out[a] = e ? e.core.length : 0 }; return { heads: out } }
  }
